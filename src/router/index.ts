...
export function backRouteCell(routeCell: RouteCell) {
  let children = [] as any[]
  children = routeCell.cellList.map((cell: RouteCell) => {
    return backRouteCell(cell)
  })
  let router = {
    path: '/',
    component: Layout,
    redirect: '/' + routeCell.name,
    name: routeCell.name,
    meta: {
      title: routeCell.name,
      icon: routeCell.icon,
      noCache: true
    }
  } as any
...
export const asyncRoutes: RouteConfig[] = [
    ...
    children: [
      {
        path: 'charts',
        component: () => import(/* webpackChunkName: "dashboard" */ '@/views/dashboard/index.vue'),
        name: 'onlineWatcherChart',
        meta: {
          title: 'onlineWatcherChart',
  ...
  },
  {
    path: '/powerControler',
    component: Layout,
    redirect: '/powerControler/index',
    meta: {
      title: 'powerControler',
      icon: 'lock',
      roles: ['admin'], // you can set roles in root nav
      alwaysShow: true // will always show the root menu
    },
  ...
  //       name: 'I18n',import { icon } from 'vue-svgicon';
